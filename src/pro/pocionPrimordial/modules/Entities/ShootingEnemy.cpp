...
ShootingEnemy::ShootingEnemy(sf::Vector2f position)
    : Enemy(position), bulletSpeed(250.0f), bulletDelay(0.5f)
{
    ...
    {
        std::cerr << "Error cargando la textura del ShootingEnemy" << std::endl;
    }

    sprite.setTexture(texture);
    sprite.setTextureRect(sf::IntRect(4, 4, 60, 75));
    sprite.setPosition(position);
    previousPosition = position;
}

void ShootingEnemy::update(float deltaTime, sf::Vector2f playerPosition, float mapWidth, float mapHeight)
{
...
void ShootingEnemy::attack(sf::Vector2f playerPosition)
{
...
const std::vector<sf::CircleShape> &ShootingEnemy::getBullets() const
{
    return bullets;
}

void ShootingEnemy::storePreviousPosition()
{
...
void ShootingEnemy::drawInterpolated(sf::RenderWindow &window, float alpha)
{
