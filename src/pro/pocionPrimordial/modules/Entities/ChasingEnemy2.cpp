#include "ChasingEnemy2.h"
#include <cmath>
#include <iostream>

ChasingEnemy2::ChasingEnemy2(sf::Vector2f position, float speed, TileMap *tileMap)
    : Enemy(position), speed(speed), tileMap(tileMap)
{
    if (!texture.loadFromFile("resources/sprites2.png"))
    {
        std::cerr << "Error cargando la textura del ChasingEnemy2" << std::endl;
    }

    sprite.setTexture(texture);
    sprite.setTextureRect(sf::IntRect(4, 3 * 87, 60, 75));
    sprite.setPosition(position);
    previousPosition = position;
}

void ChasingEnemy2::updateAnimation(float deltaTime)
{
    ...
    if (frameToggle)
    {
        sprite.setTextureRect(sf::IntRect(4, 3 * 87, frameWidth, frameHeight));
    }
    else
    {
        sprite.setTextureRect(sf::IntRect(4 + 72, 3 * 87, frameWidth, frameHeight));
    }
}

void ChasingEnemy2::update(float deltaTime, sf::Vector2f playerPosition, float mapWidth, float mapHeight)
{
    ...
    {
        sprite.setPosition(newPos);
        std::cerr << "ChasingEnemy2 sin TileMap, movido a (" << newPos.x << ", " << newPos.y << ")" << std::endl;
    }
...
void ChasingEnemy2::render(sf::RenderWindow &window)
{
    window.draw(sprite);
}

void ChasingEnemy2::storePreviousPosition()
{
    previousPosition = sprite.getPosition();
}

void ChasingEnemy2::drawInterpolated(sf::RenderWindow &window, float alpha)
{
